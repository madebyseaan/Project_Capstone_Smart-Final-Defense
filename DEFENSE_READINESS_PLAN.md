...
| `37f9301` | `npm run verify` gate (T0-3), 2 lint errors fixed, �14 documented |
...
- `npm run verify` ? **exit 0**: frontend `tsc` 0 � `vite build` ok � backend `tsc` 0 � `eslint` **0 errors** (1104 warnings) � backend suite **38 files passed / 10 skipped / 0 failed** on `smart_test_db`.
- Live smoke: **24/24 pages OK**, 0 console errors, 0 API =400.
- Rehearsed flows (real browser, rolled back afterwards): login �3, grade save, attendance save/clear, SF10 print (PDF), EOSY read-only readiness.
...
### Updated scorecard (supersedes �1)
