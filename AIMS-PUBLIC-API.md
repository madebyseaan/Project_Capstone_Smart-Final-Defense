...
**External integration auth:** Service-to-service endpoints under `/public/*` require the header `x-api-key: <EXTERNAL_API_KEY>` instead (see [External Integration](#external-integration-service-to-service)).  
...
## External Integration (Service-to-Service)

Endpoints for external subsystems (e.g. **SMART** — global gradebook) to pull granular student scores and grades from AIMS. These are the only endpoints that do **not** require a user JWT.

**Auth:** `x-api-key: <EXTERNAL_API_KEY>` — a static key configured in AIMS via the `EXTERNAL_API_KEY` environment variable. No JWT or teacher role needed, so the SMART subsystem can pull directly over the Tailscale funnel.

| | |
|---|---|
| **Auth** | `x-api-key: <EXTERNAL_API_KEY>` |
| **Unauthorized** | `401` — missing or wrong key |
| **Unconfigured** | `503` — server has no `EXTERNAL_API_KEY` set |

> **Joining to EnrollPro/SMART records:** each score row carries the AIMS user UUID (`userId`), the student's EnrollPro id (`enrollproId`), email, and full name. Use `enrollproId` to map back to the student's LRN in EnrollPro/SMART records (AIMS does not store LRN directly).

---

### `GET /public/courses/:courseId/scores`
Pull every graded score for a course — per student, per attempt. Includes quiz submissions (regular + remedial) and task submissions with raw points, dates, and attempt numbering.

| | |
|---|---|
| **Auth** | `x-api-key` |
| **Success** | `200 OK` |
| **Errors** | `404` course not found · `401` bad key · `503` unconfigured |

**Query parameters (all optional):**

| Param | Type | Description |
|---|---|---|
| `studentId` | string (uuid) | Limit to one student's scores |
| `type` | string | `QUIZ` or `TASK` — only that assessment type |
| `from` | ISO date | Only submissions `gradedAt >= from` |
| `to` | ISO date | Only submissions `gradedAt <= to` |

**Example request:**
```bash
curl -H "x-api-key: <EXTERNAL_API_KEY>" \
  "https://100.92.245.14:5000/api/v1/public/courses/4adeb111-7c78-49dd-bae3-e6721825b4a9/scores?type=QUIZ"
```
    ...
    "rows": [
      {
        ...
        "enrollproId": 21045,
        "assessmentId": "QUIZ:932ea499-4b99-4e7d-b59d-b583b05fa640",
        "quizId": "932ea499-4b99-4e7d-b59d-b583b05fa640",
        "quizTitle": "Q1 Summative",
        "type": "QUIZ",
        "category": "WW",
      ...
      {
        "submissionId": "task-sub-uuid",
        "userId": "80fddb9a-9809-47bd-9e22-bfb9e344df5d",
        "studentName": "Juan Dela Cruz",
        "studentEmail": "student1@school.edu",
        "enrollproId": 21045,
        "assessmentId": "TASK:8af41140-e787-4ceb-815d-edbe219c34f1",
        "quizId": null,
        "quizTitle": "Test YouTube Task",
        "type": "TASK",
        "category": "WW",
        "isRemedial": false,
        "sourceQuizId": null,
        ...
        "gradedAt": "2026-04-20T09:30:00.000Z"
      }
    ]
  }
}
...
> **Field semantics:**
> - `score` — normalized to 0–100: quizzes are stored as percentages; tasks are `pointsEarned / maxPoints × 100`.
> - `pointsEarned` / `maxPoints` — raw rubric points (quizzes always have `maxPoints: 100`).
> - `attemptNumber` — 1-based attempt counter per student per assessment, ordered by `startedAt`.
> - `status` — `GRADED` or `RETURNED` (only graded rows are returned).
> - `isRemedial` — `true` for remedial quiz attempts; `sourceQuizId`/`forStudentId` trace the remedial back to its source quiz and target student.
> - Only `PUBLISHED` quizzes/tasks in non-archived courses are included. Rows are ordered by student, then attempt start time.

---

### `GET /public/students/:studentId/grades`
Pull a student's quarterly grade profile across every enrolled course.

| | |
|---|---|
| **Auth** | `x-api-key` |
| **Success** | `200 OK` |
| **Errors** | `404` student not found · `401` bad key · `503` unconfigured |

**Example request:**
```bash
curl -H "x-api-key: <EXTERNAL_API_KEY>" \
  "https://100.92.245.14:5000/api/v1/public/students/80fddb9a-9809-47bd-9e22-bfb9e344df5d/grades"
```
    ...
    "courses": [
      {
        ...
        "schoolYear": "2024-2025",
        "weights": { "ww": 30, "pt": 70 },
        "categoryAverages": { "ww": 82, "pt": 90.5 },
        "initialGrade": 85,
        "quarterlyGrade": 85,
        "assessmentCount": 3,
...
> **Quarterly grade formula:**  
> $Q = \frac{WW_{avg} \times w_{ww} + PT_{avg} \times w_{pt}}{w_{ww} + w_{pt}}$  
> Only categories with at least one graded submission are included. Courses without an assigned teacher are omitted from `courses`.
...
| `GET /public/courses/:courseId/scores` | `x-api-key` | External (SMART) |
| `GET /public/students/:studentId/grades` | `x-api-key` | External (SMART) |
